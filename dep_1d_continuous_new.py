...
sim_dep_on = fokker_planck(temperature=TEMPERATURE, drag=DRAG, extent=EXTENT,
                           resolution=RESOLUTION, boundary=boundary.reflecting,
                           potential=dep_potential_1D_on_positive())
...
def update_plot(last_dep_on_interval):
    ...
    # Steady state
    ax2.plot(sim_dep_off.grid[0] / nm, steady_dep_off, color='k', ls='--', alpha=.5, label = 'Uniform pdf')
    ax3.plot(sim_dep_on.grid[0] / nm, steady_dep_off, color='k', ls='--', alpha=.5, label='Uniform pdf')

    # Initial PDF
    ax2.plot(sim_dep_off.grid[0] / nm, Pt_dep_off[0], color='red', ls='--', alpha=.3, label='Initial pdf')
    ax3.plot(sim_dep_on.grid[0] / nm, Pt_dep_on[0], color='red', ls='--', alpha=.3, label='Initial pdf')

    # interval PDF
    line2, = ax2.plot(sim_dep_off.grid[0] / nm, Pt_dep_off[0], lw=2, color='C3', label='Interval pdf')
    line3, = ax3.plot(sim_dep_on.grid[0] / nm, last_dep_on_interval, lw=2, color='C3', label='Interval pdf')
    ...
    # Header
    ax2.set_title(f'OFF\nTime: {t_dep_off[-1]:.2e} s',fontsize=16)
    ax3.set_title(f'ON\nTime: {t_dep_on[-1]:.2e} s',fontsize=16)
...
# Steady state
ax1.plot(sim_dep_on.grid[0] / nm, steady_stabilisation, color='k', ls='--', alpha=.5, label='Uniform pdf')
# Initial PDF
ax1.plot(sim_dep_on.grid[0] / nm, Pt_stabilisation[0], color='red', ls='--', alpha=.3, label='Initial pdf')
# interval PDF
line1, = ax1.plot(sim_dep_on.grid[0] / nm, Pt_stabilisation[0], lw=2, color='C3', label='Interval pdf')
# Axis
...
def update_stabilisation(i):
    if i < len(t_stabilisation):
        ...
        ax1.set_title(f'ON\nTime: {t_stabilisation[i]:.2e} s', fontsize=16)
...
def update_dep_off(i):
    if i < len(t_dep_off):
        ...
        ax2.set_title(f'OFF\nTime: {t_dep_off[i]:.2e} s', fontsize=16)
...
def update_dep_on(i):
    if i < len(t_dep_on):
        ...
        ax3.set_title(f'ON\nTime: {t_dep_on[i]:.2e} s', fontsize=16)
        if i == len(t_dep_on) - 1:
            iterate_simulation()
        return [line3]
