...
def dep_potential_1D_on_positive():
   ...
   # Clausius-Mossotti factor
    ...
    shift_value = 0.0
    scale_value = 0.00000000002                   # 1e-20     0.035
    max_potential = 0.000000000000000001                      # +-100nm
    ...
    def potential2(*args):
        ...
        for i, arg in enumerate(args):
            ...
            range3 = (arg >= -0.9e-7) & (arg <= 0.9e-7)
            U[range3] = 1e-5 * arg[range3] ** 2 + 1e-20
            U[~(range1|range2|range3)] = scale_value * (shift_value - 2 * 5 * np.pi * eps0 * permittivity_medium * q *  radius ** 3 * 1 * arg[~(range1|range2|range3)] ** (-4))
        return U
